...
\author
  {
  Conrad Sanderson~{$^{\dagger\diamond\ast}$} and Ryan Curtin~{$^{\ddagger\ast}$}\\
  ~\\
  
  {\normalsize{$^\dagger$}  Data61, CSIRO, Australia}\\
  {\normalsize{$^\ddagger$} Symantec Corporation, USA}\\
  {\normalsize{$^\diamond$} University of Queensland, Australia}\\
  {\normalsize{$^\ast$}     Arroyo Consortium}\\
  }
