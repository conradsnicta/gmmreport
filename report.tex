...
\scalebox{0.925}{An Open Source C++ Implementation of Multi-Threaded Gaussian~Mixture~Models, k-Means and Expectation Maximisation.}\\
International Conference on Signal Processing and Communication Systems, 2017.
%\href{http://dx.doi.org/TODO}{http://dx.doi.org/TODO} % TODO: insert link to DOI
\end{small}
\end{minipage}
...
\item
Associated C++ source code: \href{http://arma.sourceforge.net}{http://arma.sourceforge.net}
...
\item
{\bf M.sum\_log\_p(X)}\\
return a scalar (double precision floating point value) representing the sum of log-likelihoods of all column vectors in matrix {\bf X}

\item
{\bf M.sum\_log\_p(X, g)}\\
return a scalar (double precision floating point value) representing the sum of log-likelihoods of all column vectors in matrix {\bf X},
according to Gaussian with index~{\bf g}  (specified as an unsigned integer of type {\it uword})
