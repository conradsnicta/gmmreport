...
\item
{\bf M.log\_p(V)} - return a scalar representing log-likelihood of vector {\bf V}

\item
{\bf M.log\_p(V, g)} - return a scalar representing log-likelihood of vector {\bf V} according to Gaussian {\bf g}

\item
{\bf M.log\_p(X)} - return a row vector containing log-likelihoods of each column vector in matrix {\bf X}
